...
def acquisition_ei(mu, sigma, y_max, xi=0.01, ei_floor_ratio=0.05):
    ...
    # ---- HANDLE ZERO / NEGATIVE EI ----
    ei = np.maximum(ei, 0.0)

    # ---- EI FLOOR (PREVENT COLLAPSE) ----
    if np.any(ei > 0):
        ei_floor = ei_floor_ratio * np.max(ei)
        ei = np.maximum(ei, ei_floor)
    # ----------------------------------

    return ei


def acquisition_ei_noisy(mu, sigma, y_max_est, xi=0.01, ei_floor_ratio=0.05):
     # y_max_est is not fixed, it uses UCB or posterior max estimate
    with np.errstate(divide='ignore', invalid='ignore'):
        sigma = np.maximum(sigma, 1e-9)
        z = (mu - y_max_est - xi) / sigma
        ei = (mu - y_max_est - xi) * stats.norm.cdf(z) + sigma * stats.norm.pdf(z)
...
def acquisition_portfolio(mu, sigma, y_max, iteration, is_noisy=False):
    """
    Simple Portfolio: average of UCB, EI, and Thompson.
    Uses noisy EI when is_noisy=True.
    """
    ucb = acquisition_ucb(mu, sigma, iteration)
    if is_noisy:
        y_max_est = np.max(mu + 1.96 * sigma)
        ei = acquisition_ei_noisy(mu, sigma, y_max_est)
    else:
...
def select_acquisition(acq_name, mu, sigma=None, iteration=None, y_max=None, model_type="GP",is_noisy=False, **kwargs):
    """
    ...
    if acq_name == "UCB":
        ...
        if is_noisy:
            y_max_est = np.max(mu + 1.96 * sigma)
            return acquisition_ei_noisy(mu, sigma, y_max_est, xi=kwargs.get("xi", 0.01))
        else:
    ...
    #     return acquisition_ei(mu, sigma, y_max, xi=kwargs.get("xi", 0.01))
        ...
        if y_max is None or iteration is None:
            raise ValueError("y_max and iteration must be provided for PORTFOLIO")
        return acquisition_portfolio(mu, sigma, y_max, iteration)
    else:
...
def log_all_acquisitions(mu_all, sigma_all, y_max, gp_health, iteration):
    ...
    return acq_values
# 
def evaluate_acquisition(acq_name, mu, sigma=None, y_max=None, iteration=None, total_iterations=None,is_noisy = None, **kwargs):
    """
    Evaluate an acquisition function using dynamic parameters from the strategy.

    Parameters
    ----------
    acq_name : str
        Acquisition function name ("UCB", "EI", "PI", etc.)
    mu : float
        Predicted mean
    sigma : float, optional
        Predicted standard deviation
    y_max : float, optional
        Current best observed value (for EI, PI, KG)
    iteration : int, optional
        Current iteration (for UCB or time-dependent acquisitions)
    total_iterations : int, optional
        Total iterations (if needed for dynamic acquisitions)
    **kwargs : dict
        Additional acquisition parameters like kappa, xi, eta, beta0, n_samples, etc.
    ...
    # For UCB, ensure iteration is passed
    if acq_name == "UCB" and iteration is None:
        raise ValueError("iteration must be provided for UCB acquisition")

    # Forward all kwargs to select_acquisition
    val = select_acquisition(
        acq_name,
        mu,
        sigma=sigma,
        iteration=iteration,
...
function_strategy_map_week9 = {
    1: {"type": "dense_exploit", "acq": [{"name": "UCB"}]},
    2: {"type": "global_explore", "acq": [{"name": "UCB"}, {"name": "PORTFOLIO"}]},
    3: {"type": "refinement", "acq": [{"name": "UCB"}, {"name": "EI"}]},
    4: {"type": "dense_exploit", "acq": [{"name": "EI"}, {"name": "UCB"}]},
    5: {"type": "mixed", "acq": [{"name": "EI"}, {"name": "UCB"}]},
    6: {"type": "local_exploit", "acq": [{"name": "PI"}, {"name": "EI"}]},
    7: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "THOMPSON"}]},
    8: {"type": "refinement", "acq": [{"name": "Thompson"}, {"name": "EI"}]}
}
# function_strategy_map_week10 ={
#     1: {
#         "type": "dense_exploit",          # focus on exploiting high-potential regions
#         "acq": [{"name": "UCB", "beta": 3}],  # slightly lower beta than week 9 to avoid huge spikes
#     },
#     2: {"type": "global_explore", "acq": [{"name": "UCB"}, {"name": "PORTFOLIO"}]},
#     3: {"type": "refinement", "acq": [{"name": "UCB"}, {"name": "EI"}]},
#     4: {"type": "dense_exploit", "acq": [{"name": "EI"}, {"name": "UCB"}]},
...
# }
# function_strategy_map_week10 = {
#     1: {"type": "dense_exploit", "acq": [{"name": "UCB", "beta": 0.5}]},
#     2: {"type": "global_explore", "acq": [{"name": "EI"}, {"name": "UCB", "beta": 0.5}]},
#     3: {"type": "refinement", "acq": [{"name": "EI"}, {"name": "UCB", "beta": 0.5}]},
#     4: {"type": "dense_exploit", "acq": [{"name": "EI"}, {"name": "UCB", "beta": 0.5}]},
#     5: {"type": "local_exploit", "acq": [{"name": "EI"}]},
#     6: {"type": "local_exploit", "acq": [{"name": "EI"}]},
#     7: {"type": "exploit_only", "acq": [{"name": "UCB", "beta": 0.5}]},
#     8: {"type": "refinement", "acq": [{"name": "EI"}]}
# }
# function_strategy_map_week10 = {
#     1: {"type": "dense_exploit", "acq": [{"name": "UCB", "beta": 1.0}]},   # focus exploitation
#     2: {"type": "global_explore", "acq": [{"name": "UCB", "beta": 2.0}, {"name": "PORTFOLIO"}]},  # explore more
#     3: {"type": "refinement", "acq": [{"name": "UCB", "beta": 1.2}, {"name": "EI"}]},
#     4: {"type": "dense_exploit", "acq": [{"name": "EI"}, {"name": "UCB", "beta": 1.0}]},
#     5: {"type": "mixed", "acq": [{"name": "EI"}, {"name": "UCB", "beta": 1.0}]},
#     6: {"type": "local_exploit", "acq": [{"name": "PI"}, {"name": "EI"}]},
#     7: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "THOMPSON"}]},
#     8: {"type": "refinement", "acq": [{"name": "Thompson"}, {"name": "EI"}]}
# }
function_strategy_map_week10 = {
    1: {"type": "dense_exploit", "acq": [{"name": "UCB", "beta": 1.0}]},   # flat, exploit any small positive
    2: {"type": "global_explore", "acq": [{"name": "UCB", "beta": 2.0}, {"name": "PORTFOLIO"}]},  # explore more for scattered peaks
    3: {"type": "refinement", "acq": [{"name": "UCB", "beta": 1.2}, {"name": "EI"}]},  # refine after modest exploration
    4: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}]},  # multimodal, explore first
    5: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}]},  # rare high peaks, explore more
    6: {"type": "local_exploit", "acq": [{"name": "PI"}, {"name": "EI"}]},  # modest peaks, local refinement
    7: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 0.8}]},  # explore then climb higher
    8: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 1.0}]}  # high-dim, mix exploration & refinement
}
function_strategy_map_week11 = {
    1: {"type": "dense_exploit", "acq": [{"name": "UCB", "beta": 1.0}]},   # flat, exploit any small positive
    2: {"type": "global_explore", "acq": [{"name": "UCB", "beta": 2.0}, {"name": "PORTFOLIO"}]},  # explore more for scattered peaks
    3: {"type": "refinement", "acq": [{"name": "UCB", "beta": 1.2}, {"name": "EI"}]},  # refine after modest exploration
    4: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}]},  # multimodal, explore first
    5: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}]},  # rare high peaks, explore more
    6: {"type": "mixed", "acq": [{"name": "EI"}, {"name": "UCB", "kappa": 2.5}]},  # modest peaks, local refinement
    7: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 0.8}]},  # explore then climb higher
    8: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 1.0}]}  # high-dim, mix exploration & refinement
}
function_strategy_map_week12 = {
    1: {"type": "dense_exploit", "acq": [{"name": "EI", "beta": 1.0}]},   # flat, exploit any small positive
    2: {"type": "noisy_explore", "acq": [{"name": "UCB", "beta": 3.5}, {"name": "PORTFOLIO"}]},  # explore more for scattered peaks
    3: {"type": "refinement", "acq": [{"name": "UCB", "beta": 1.2}, {"name": "EI"}]},  # refine after modest exploration
    4: {"type": "mixed", "acq": [{"name": "EI"}, {"name": "UCB", "beta": 2.0}]} , # multimodal, explore first
    5: {"type": "global_explore", "acq": [{"name": "UCB", "beta": 2.0},{"name": "PORTFOLIO"}, {"name": "EI"}]},  #  explore more
    6: {"type": "mixed", "acq": [{"name": "EI"}, {"name": "UCB", "kappa": 2.5}]},  # modest peaks, local refinement
    7: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 0.8}]},  # explore then climb higher
    8: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 1.0}]}  # high-dim, mix exploration & refinement
}
function_strategy_map_week13 = {
    1: {"type": "global_explore", "acq": [{"name": "EI", "beta": 1.0}]},   # flat, exploit any small positive
    2: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "UCB", "kappa": 1.0}]},  
    3: {"type": "refinement", "acq": [{"name": "UCB", "beta": 1.2}, {"name": "EI"}]},  # refine after modest exploration
    4: {"type": "refinement", "acq": [{"name": "UCB", "beta": 1.2}, {"name": "EI"}]}, 
    5: {"type": "explore_then_exploit", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}]},
    6: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "UCB", "kappa": 1.0}]},  
    7: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 0.8}]},  # explore then climb higher
    8: {"type": "refinement", "acq": [{"name": "PORTFOLIO"}, {"name": "EI"}, {"name": "UCB", "beta": 1.0}]}  # high-dim, mix exploration & refinement
}
...
def generate_candidates_by_strategy(func_id,strat_type, gp, X_train_scaled, iteration, total_iterations):
    ...
    if strategy_type == "dense_exploit":
        ...
        noise = params.get("noise",0.25)# 0.12)
        ...
        explore_ratio = params.get("explore_ratio",0.5) #0.3)
        radius_scale = params.get("radius_scale", 0.3)#0.20)
    ...
    elif strategy_type == "noisy_explore":
        n_rand = int(n_candidates * 0.6)
        n_unc = n_candidates - n_rand

        rand_cand = np.random.rand(n_rand, X_train_scaled.shape[1])
        unc_cand = generate_uncertainty_candidates(gp, X_train_scaled, n=n_unc)

        return np.vstack([rand_cand, unc_cand])
