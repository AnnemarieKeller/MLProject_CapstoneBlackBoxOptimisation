...
def build_dynamic_kernel_hybrid(
    X_train=None,
    y_train=None,
    config=None,
    kernel_override=None,
    ...
    if kernel_type in ("rbf+matern", "matern+rbf", "hybrid"):
        base1 = RBF(length_scale=length_init,
                    length_scale_bounds=length_bounds)
        nu = cfg.get("nu", 2.5)
        base2 = Matern(length_scale=length_init,
                       length_scale_bounds=length_bounds,
                       nu=nu)
        base_kernel = base1 + base2
    elif kernel_type == "matern":
        nu = cfg.get("nu", 2.5)
        base_kernel = Matern(
            length_scale=length_init,
